...
const Cart: React.FC<CartProps> = ({ items, onClose, onUpdateQuantity, onClearCart }) => {
  ...
  const sendOrderToWhatsApp = async () => {
    ...
    if (!isAddressValid) {
      alert('⚠️ Por favor, preencha o endereço para entrega.');
      return;
    }

    setIsSubmitting(true);
    try {
      // REGISTRO NO BANCO DE DADOS (DASHBOARD)
      const { error } = await supabase
        ...
        .insert({
          ...
          customer_address: `${street}, ${neighborhood}`
        });

      if (error) console.error("Erro ao registrar pedido no dashboard:", error);
...
_Olá! Gostaria de confirmar meu pedido feito pelo site._`;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slideInRight">
                ...
                <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      ...
                      <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Ex: João da Silva" className="cart-input-field" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Rua e Número *</label>
                      <input type="text" value={street} onChange={(e) => setStreet(e.target.value)} placeholder="Ex: Rua Principal, 100" className="cart-input-field" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Bairro *</label>
                      <input type="text" value={neighborhood} onChange={(e) => setNeighborhood(e.target.value)} placeholder="Ex: Campo Lindo" className="cart-input-field" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Ponto de Referência</label>
                      <input type="text" value={referencePoint} onChange={(e) => setReferencePoint(e.target.value)} placeholder="Ex: Próximo ao Colégio X" className="cart-input-field" />
                    </div>
                  </div>
                </div>
              </>
        ...
        {items.length > 0 && (
          <div className="p-6 bg-white border-t border-gray-100 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
            ...
            >
              {isSubmitting ? <i className="fas fa-spinner fa-spin"></i> : <i className="fab fa-whatsapp text-2xl"></i>}
              {isSubmitting ? 'PROCESSANDO...' : (isAddressValid ? 'FINALIZAR NO WHATSAPP' : 'COMPLETE O ENDEREÇO')}
            </button>
          </div>
        )}
      </div>
      ...
      <style>{`
        .cart-input-field { width: 100%; background: #fdfdfd; border: 2px solid #f3f4f6; padding: 10px 14px; border-radius: 0.8rem; font-weight: 700; font-size: 0.8rem; color: #002395; outline: none; transition: border-color 0.2s; }
